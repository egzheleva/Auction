...
export default function EnhancedTable() {
  ...
  useEffect(() => {
    async function fetchData() {
      setUsers(await USERS_API.getUsers());
    }
    fetchData();
  }, [])
  ...
  async function onDeleteUser() {
    ...
    setUsers(users.filter(c => c.id !== deletedUser.id));
  }

  return (
    <div className={classes.root}>
      <Paper className={classes.paper}>
        <EnhancedTableToolbar numSelected={selected.length} handleDeleteUser={onDeleteUser}/>
        <TableContainer>
          <Table
